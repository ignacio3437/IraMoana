...
genetic.diversity.mtDNA.db<-function(ipdb=ipdb, basic_diversity = T, sequence_diversity = T, coverage_calc = F, coverage_correction = F, minseqs = 6, minsamps = 3, mintotalseqs = 0, ABGD=F,regionalization = c("sample","fn100id", "fn500id", "ECOREGION", "PROVINCE", "REALM", "EEZ"), keep_all_gsls=F, mincoverage = 0.4, hill.number = 0){
  ...
  for(gsl in esu_loci){ #gsl<-"Acanthaster_planci_CO1" 
    ...
    spseqs_wc<-cbind(seq.gtype@data$strata,seq.gtype@data)
    ...
    pop.data<-data.frame(popname=sort(unique(seq.gtype@data$strata)) )
    
    #loop through the populations in seq.gtype
    populations<-as.character(unique(seq.gtype@data$strata))
    ...
    if(basic_diversity == T){
      ...
      for (p in 1:length(populations)) {
        ...
        pop.data[p, "SWdiversity"]<-shannon.wiener.diversity(singlepop@data$haplotype) #Shannon-Wiener Diversity based on the modified diversity function below
        #Effective number of haplotpyes
        pop.data[p, "EffNumHaplos"]<-1/(1-uncorrected.diversity(singlepop@data$haplotype)) #No sample size correction - based on Crow & Kimura 1964, eq 21. See also Jost 2008 eq 5
      }
